...
(defn maximo-lista
  ...
  [coll]
  (reduce utils/maximo (first coll) (rest coll))
)
...
(defn fibonacci-clj
  ...
  (if (= n 0)
    ...
    (if (= n 1)
      1
      (+ (fibonacci-clj (- n 1)) (fibonacci-clj (- n 2)))
    )
  )
)
...
(defn agrupar-por-tipo
  ...
  (reduce
    (fn [acc registro]
      (update acc (:tipo registro) #(conj (or % []) registro))
    )
    {}
    registros
  )
...
(defn aplicar-descuento
  ...
  [productos]
  (map
    (fn [elem] (assoc elem :precio (* (:precio elem) 0.9)))
    productos
  )
)
